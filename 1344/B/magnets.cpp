...
bool check_row_col(vector<vector<int>> &grid){
  int n = grid.size();
  int m = grid[0].size();
...
int main(){
  ...
  bool is_row_col = check_row_col(grid);
 
  // cout << "convexity: " << check_convexity(grid) << endl;
  // cout << "row/col: " << check_row_col(grid) << endl;
  // cout << "num_islands: " << num_islands(grid) << endl;
 
  if(all_white || (is_convex && is_row_col))
